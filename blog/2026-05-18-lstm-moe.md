...
## 7. Forcing specialisation: a 2-domain experiment
...
Notebook: [03-lstm-moe-domain-split](https://github.com/createcentury/jax-flax-optax-lab/blob/main/notebooks/03-lstm-moe-domain-split.ipynb). Two outcomes to read for:

- **Strong asymmetry** (e.g. 80/20 split on each expert with the per-domain pairs flipped) → experts have actually carved up the input space by domain. Generations switch routing modes cleanly when the prompt switches.
- **No asymmetry** (both columns near 50%) → routing is balanced globally but uninformative per-token. The model is solving the task without using experts as a specialisation device at all; the FFN is just *replicated four ways with a noisy gate*.

The latter outcome is more common than people advertising MoE results admit, especially at small $E$. A real win requires either many more experts, an explicit auxiliary signal that rewards specialisation, or some form of routing that the LM has to use to fit the data (e.g. capacity-constrained dispatch that forces choices). 1500 steps and 4 experts is a setup where the question is interesting precisely because the answer isn't predetermined.
